...
배치 크기는 GUI 드롭다운에서 `40`, `60`, `80` 중 고를 수 있습니다. `40`은 안정 기본값, `60`은 속도와 안정성의 타협, `80`은 빠르지만 긴 설명문이 많은 모드에서 JSON 응답 오류 가능성이 더 큽니다.
...
보고서에는 원문, 기존 번역, 후보 번역, 토큰 누락 여부, 비정상 개행 후보 여부, 한글 포함 여부, 안전 적용 가능 여부가 같이 들어갑니다. GUI에서는 비교/검토 모드가 끝나면 검토 결과 폴더가 자동으로 열리고, `검토 폴더 열기` 버튼으로 다시 열 수 있습니다. 실제 비교표는 `_TranslationAudit\*-comparison.csv`입니다.
