...
기준 시각: 2026-07-13, v1.0.0 C# 릴리스 후보

## 현재 구현

| 영역 | 상태 | 코드 근거 |
|---|---|---|
| 실행 | C# WinForms 단일 EXE가 직접 시작하며 사용자 런타임 PowerShell 의존성이 없다. | `src/RimWorldAiTranslator.App/Program.cs`, `MainForm.cs`, `build-package.ps1` |
...
| 로컬 적용 | 검토됨 또는 번역됨까지 선택해 Korean XML에 기록하며 토큰·경로·중복을 재검증하고 실패 시 롤백한다. | `Core/Apply/ReviewApplyService.cs`, `Apply.*` 회귀 |
...
- 오프라인 C# 회귀: 30/30 PASS. 실제 사용자 데이터와 외부 네트워크를 사용하지 않는다.
- 실제 Workshop 모드를 읽기 전용으로 열어 선택 언어 원문, RMK 참고 번역, Def Class/Node와 원문 갱신 중 중지 버튼을 확인했다.
- win-x64 배포본을 격리 데이터 루트에서 두 번 실행해 정상 창 표시, 정상 종료, ExitCode 0과 PowerShell 자식 0개를 확인했다.
...
현재 확인된 미해결 P0 데이터 손실·파일 손상·키 노출·실행 불가 결함은 없다. 외부 제공자 실환경과 폭넓은 DPI 조합은 조사·수동 검증이 남는다.
