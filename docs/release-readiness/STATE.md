...
- Candidate current: preserved dirty-tree candidate plus Phase 04-10, legacy PowerShell data compatibility, direct API-key editor and atomic startup presentation fixes. The exact RC was built from the earlier frozen 250-file package-build input snapshot v5 (combined SHA-256 `C279F6F6C87634D95035C3D17821770DE0703649CCD33A3A55B123695683A006`) and is superseded by the current source changes; no package or release asset was rebuilt.
...
| 4 C# migration / PowerShell zero | PASS | C# App/Core/Native and C# glossary/package tooling; active source and exact RC script count 0 | unstaged index still records two tracked-deleted legacy scripts |
| 5 Data/translation compatibility | PASS current source | Actual legacy project copy opened read-only; project v2/settings v3/review v5/marker v1 and RMK first-wins compatibility; source analysis, explicit backed-up migration/restart, corrupt-data rejection and anonymized regression fixture | actual Excel remains manual |
...
| 8 Reliability/performance/tests | BLOCKED current | Strict 0/0 and all compatibility/cleanup/UI harness gates pass; current full suite is 82/83 | `Phase08.ForcedExitRecovery` 1,000-target journal evidence took 70.539s against the 60s contract; performance work was out of scope and the gate was not relaxed |
